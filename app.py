...
def send_telegram_report(bot_token, chat_id, message, chart_fig=None):
...
# 🌐 ការកំណត់ cấu hình trang Web Interface แบบ Premium
...
# ------------------------------------------------------------------
# SIDEBAR CONTROL PANEL (សុវត្ថិភាពខ្ពស់ វាយបញ្ចូលតាមការប្រើប្រាស់ជាក់ស្តែង)
# ------------------------------------------------------------------
st.sidebar.markdown("### 🔑 TELEGRAM CREDENTIALS")
bot_token_input = st.sidebar.text_input("Telegram Bot Token:", type="password", help="បញ្ចូល Token ដែលទទួលបានពី @BotFather")
chat_id_input = st.sidebar.text_input("Telegram Chat ID:", help="ឧទាហរណ៍៖ -5079160685")
...
# ការរៀបចំទិន្នន័យ (អាន File បើមាន បើគ្មានទេប្រើទិន្នន័យគំរូស្វ័យប្រវត្តិ)
df = None
if uploaded_file is not None:
    try:
        ...
        st.success(f"⚡ **System Connected:** `{uploaded_file.name}` loaded successfully.")
    except Exception as e:
        st.error(f"❌ System Error Loading File: {e}")

# ប្រសិនបើគ្មាន File ទេ បង្កើតទិន្នន័យគំរូជូន ដើម្បីឱ្យ Dashboard ដំណើរការជានិច្ច
if df is None:
    st.info("💡 មិនទាន់មានឯកសារបញ្ចូលឡើយ។ ប្រព័ន្ធកំពុងបង្ហាញទិន្នន័យគំរូ (Sample Data) សម្រាប់ធ្វើ Demo ជូន។")
    df = pd.DataFrame({
        'តំបន់ (Region)': ['ភ្នំពេញ', 'សៀមរាប', 'ព្រះសីហនុ', 'បាត់ដំបង', 'កំពត'],
        'ចំណូលលក់ (Revenue)': [85000, 42000, 56000, 23000, 19000],
        'ចំនួនការបញ្ជាទិញ (Orders)': [340, 180, 210, 110, 95]
    })

# ទាញយកឈ្មោះជួរឈរ (Columns List)
...
st.sidebar.write("---")
st.sidebar.markdown("<small style='color:#64748B;'>Engine Status: Active 🟢</small>", unsafe_allow_html=True)
...
with tab2:
    ...
    if st.button("📤 Transmission Report to Clients Telegram"):
        # 🛑 លក្ខខណ្ឌត្រួតពិនិត្យសុវត្ថិភាពទិន្នន័យបញ្ចូល
        if not bot_token_input or not chat_id_input:
            st.warning("⚠️ សូមបំពេញព័ត៌មាន Telegram Bot Token និង Chat ID នៅក្នុងម៉ឺនុយ Control Panel (ខាងឆ្វេងដៃ) ជាមុនសិន!")
        else:
            file_display_name = uploaded_file.name if uploaded_file is not None else "Sample_Demo_Dataset.csv"
            summary_text = (
                f"📊 *🔴 LIVE ENTERPRISE DATA REPORT* 📊\n\n"
                f"📂 *Dataset Name:* `{file_display_name}`\n"
            ...
            with st.spinner("Transmitting encrypted summary and high-res chart..."):
                success = send_telegram_report(bot_token_input, chat_id_input, summary_text, chart_fig=fig)
                if success:
                    st.balloons()
                    st.success("🚀 **Dispatched Successfully!** High-resolution chart sent to client's Telegram channels.")
                else:
                    st.error("❌ **Failed!** Verify network rules, Bot token, or Chat ID connection details.")